...
//    $section->addText('PO-nummer: '.$user_po,array(),$pstyle);
	...
	//$weeks = UniHr_DB_HourRegistration::get_grouped_hour_totals_per_week_by_customer($week_from, $week_till, $year);
	...
	foreach ($weeks as $week){
		...
		*/
		...
		$label = sprintf("Werkuren week %s %s ".$user_po.PHP_EOL."(Tarief %s p/u)" ,$week->week,$user_name,$currancy.number_format_i18n($hourrate_customer,2));
		...
		foreach ($additional_hours as $additional_hour){
			...
			*/
			if($additional_hour->total_additional>0&&!in_array($additional_hour->registration_type,$exclude_types)){
				$factor = 1;
